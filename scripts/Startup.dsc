...
YAMLConfigCreator:
    type: task
    definitions: useSavedConfig[ElementTag(Boolean)]
    description:
    ...
    - encoded config file saved to the kingdoms.encodedConfig flag.
    - Note: the kingdoms.encodedConfig flag will be ignored if it's not a binary type.
    ...
    ## Note: the kingdoms.encodedConfig flag will be ignored if it's not a binary type.
    ##
    ## useSavedConfig : [ElementTag<Boolean>]
    ##
    ## >>> [Void]
...
DEBUG_SaveCurrentConfig:
    ...
    ## to the 'kingdoms.encodedConfig' flag.
    ##
    ## >>> [Void]
