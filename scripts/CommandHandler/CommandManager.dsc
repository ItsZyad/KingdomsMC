...
# @ ----------------------------------------------------------
##
## Scripts in this file all relate to 0Tick's automated command argument manager for Denizen
## command scripts. Only admin and dev-related command scripts should utilize this command engine.
##
## @Author: @0tickpulse / 0TickPulse#0296
## @Editor: Zyad (@itszyad / ITSZYAD#9280)
## @Date: Nov 2022
## @Edited: Jan 2024
## @Script Ver: v0.10
##
## Note: This module is still uncompliant with Kingdoms contribution standards. Requires Refactor.
##
## ------------------------------------------END HEADER-------------------------------------------
...
GenerateUsage_CM:
    ...
    definitions: script
    script:
    - define colors <static[<script[CommandManager_Data].parsed_key[formatting.colors]>]>
    - define symbols <static[<script[CommandManager_Data].parsed_key[formatting.symbols]>]>
    - define output /<[script].data_key[name]>

    - if <[script].data_key[data.enable_subcommands].if_null[false]>:
        - define subcommand_strings <list>

        - foreach <[script].proc[GetSubcommandData_CM]> key:key as:args:
            - inject CommandManager.GenerateArgMaps
            - define "subcommand_strings:->:<[colors.literal_arg_text]><[key]> <[args].parse_value_tag[<[parse_key].proc[GenerateSingularUsage_CM].context[<[parse_value]>]>].values.separated_by[ ]>"
    ...
    - define output "<[output]> <[args].parse_value_tag[<[parse_key].proc[GenerateSingularUsage_CM].context[<[parse_value]>]>].values.separated_by[ ]>"
    - determine <[output]>
...
ArgMapGenerator_CM:
    ...
    - if <[template_map.template].exists>:
        - define template_map <[template_key].proc[ArgMapGenerator_CM].context[<script[CommandManager_Data].data_key[arg templates.<[template_map.template]>].include[<[template_map].exclude[template]>]>].values.first>

    - define output <map.with[<[template_map.name].if_null[<[template_key]>]>].as[<[template_map]>]>
    - determine <[output]>


GenerateColoredChoices_CM:
    type: procedure
    debug: false
...
GenerateSingularUsage_CM:
    ...
    - define colors <static[<script[CommandManager_Data].parsed_key[formatting.colors]>]>
    - define symbols <static[<script[CommandManager_Data].parsed_key[formatting.symbols]>]>
    - define sep <script[CommandManager_Data].parsed_key[formatting.lang.prefixed_value_separator]>
    - define output <empty>
    ...
    # Usage text
    - define usage_text <[arg.usage text.auto format].if_null[false].if_true[<[arg.usage text.list].proc[GenerateColoredChoices_CM].context[<[arg.type].if_null[prefixed]>]>].if_false[<[arg.usage text.text].parsed.if_null[<[symbols.nonliteral_arg_prefix]><[colors.nonliteral_arg_text]><[name]><[symbols.nonliteral_arg_suffix]>]>]>

    - if <[arg.type].if_null[null]> == linear:
        - define output <[output]><[arg.spread].if_null[false].if_true[<script[CommandManager_Data].parsed_key[formatting.symbols.spread]>].if_false[]><[usage_text]>
...
FormattedSameFileHelp_CM:
    ...
    - define hline <script[CommandManager_Data].parsed_key[formatting.lang.hline]>

    - if <[page]> > 1:
        - clickable until:10m save:previous_page:
            - run FormattedSameFileHelp_CM def.script:<[script]> def.size:<[size]> def.page:<[page].sub[1]> def.title:<[title]>

        - define left_arrow <script[CommandManager_Data].parsed_key[formatting.lang.formatted_help_same_file.left_arrow].on_click[<entry[previous_page].command>]>

    - if <[page]> < <[pages]>:
        - clickable until:10m save:next_page:
            - run FormattedSameFileHelp_CM def.script:<[script]> def.size:<[size]> def.page:<[page].add[1]> def.title:<[title]>

        - define right_arrow <script[CommandManager_Data].parsed_key[formatting.lang.formatted_help_same_file.right_arrow].on_click[<entry[next_page].command>]>

    - define page_message <script[CommandManager_Data].parsed_key[formatting.lang.formatted_help_same_file.page_text]>
...
CommandManager:
    ...
    FormattedHelpSameFile:
    - run FormattedSameFileHelp_CM def.script:<queue.script> def.size:10 def.page:<[arg.page].if_null[1]> def.title:<[title].if_null[]>

    #------------------------------------------------------------------------------------------

    FlagArgs:
    - define FlagArgs <map.with[linear_args].as[<list>].with[prefixed_args].as[<map>]>
    - define sep <script[CommandManager_Data].parsed_key[formatting.lang.prefixed_value_separator]>

    - foreach <context.args> as:arg:
        - if !<[arg].starts_with[-]>:
            - define FlagArgs.linear_args:->:<[arg]>
            - foreach next

        - define FlagArgs.prefixed_args <[FlagArgs.prefixed_args].with[<[arg].after[-].before[<[sep]>]>].as[<[arg].contains_text[<[sep]>].if_true[<[arg].after[<[sep]>]>].if_false[true]>]>

    #- define FlagArgs.linear_args <context.args.filter[starts_with[-].not]>
    #- define FlagArgs.prefixed_args <context.args.filter[starts_with[-]].parse_tag[<list_single[<[parse_value].after[-].before[<[sep]>]>].include_single[<[parse_value].contains_text[<[sep]>].if_true[<[parse_value].after[<[sep]>]>].if_false[true]>]>]>
    #- narrate <[FlagArgs].to_yaml>
    ...
    - if !<queue.definitions.contains[args]>:
        - define subcommands_data <[script].proc[GetSubcommandData_CM]>
        ...
        - if <[subcommands_enabled]>:
            - define subcommand <[FlagArgs.linear_args].if_null[<list>].first.if_null[null]>
            ...
            - else:
                - if <[subcommands_data].keys> !contains <[subcommand]>:
                    ...
                    - define tickutil_commands.ArgManager.close_match <[subcommands_data].keys.closest_to[<[subcommand]>]>

                    - if <[tickutil_commands.ArgManager.close_match].to_lowercase.difference[<[subcommand].to_lowercase>]> <= 3 && <[tickutil_commands.ArgManager.close_match]> != <empty>:
                        - define tickutil_commands.ArgManager.close_match_text " Did you mean '<white><[tickutil_commands.ArgManager.close_match]>?<&[error]>"

                    - define "tickutil_commands.ArgManager.error_messages:->:<&[error]>Unknown subcommand '<white><[subcommand]><&[error]>'!<[tickutil_commands.ArgManager.close_match_text].if_null[]>"
                    - goto fatal_error
    ...
    - define args <[args].parse_value_tag[<[parse_key].proc[ArgMapGenerator_CM].context[<[parse_value].as[map]>]>].values.merge_maps.exclude[_permission]>
    ...
    - inject CommandManager.FlagArgs
    - inject CommandManager.GenerateArgMaps

    - define completions <list>
    - define unfiltered_completions <list>

    - define current_arg <[FlagArgs.linear_args].if_null[<list>].size.max[1]>

    - if <[FlagArgs.linear_args].if_null[<list>].size> >= 1 && <context.raw_args.ends_with[ ]>:
        - define current_arg:++
    ...
    - if <[current_linear_arg].exists>:
        - define will_complete_with <[current_linear_arg].get[2].get[tab completes].parsed.if_null[<[current_linear_arg].get[1].proc[GenerateSingularUsage_CM].context[<[current_linear_arg].get[2]>].strip_color>].as[list]>
        - define usage <[current_linear_arg].get[1].proc[GenerateSingularUsage_CM].context[<[current_linear_arg].get[2]>].strip_color>

        - if <[FlagArgs.linear_args].exists>:
            - define value <[FlagArgs.linear_args].get[<[current_arg]>].if_null[]>
    ...
    - define prefixed <[prefixed_args].filter_tag[<context.raw_args.ends_with[ ].if_true[<[FlagArgs.prefixed_args].if_null[<map>]>].if_false[<[FlagArgs.prefixed_args].if_null[<map>].exclude[<context.args.last.after[-].before[<[sep]>].if_null[]>]>].contains[<[filter_key]>].not>]>
    ...
    - foreach <[prefixed]> key:argname as:map:
        ...
        - if <context.args.last.starts_with[<[prefix]>].if_null[false]>:
            - if <[FlagArgs.prefixed_args].exists>:
                - if <[FlagArgs.prefixed_args].contains[<[argname]>]>:
                    - define value <[FlagArgs.prefixed_args.<[argname]>].if_null[]>

                    - if <[will_complete_with].filter[starts_with[<[prefix]><[value]>]].any>:
                        - define completions:|:<[will_complete_with]>

                    - else if <[map.accepted].parsed.if_null[true]>:
                        - define completions:|:<[argname].proc[GenerateSingularUsage_CM].context[<[map]>].strip_color>

                    - else:
                        - definemap err_completion:
                            expected: <[argname].proc[GenerateSingularUsage_CM].context[<[map]>].strip_color>
                            actual: <[prefix]><[value]>
    ...
    - mark fatal_error
    - determine <[tickutil_commands.ArgManager.error_messages].if_null[<list>]>
    ...
    - define tickutil_commands.ArgManager.linear_args <[args].filter_tag[<[filter_value.type].equals[linear].if_null[true]>]>
    - define tickutil_commands.ArgManager.error_messages <list>

    - if <[subcommands_enabled]> && <[subcommand]> == null:
        - define "tickutil_commands.ArgManager.error_messages:|:No subcommand was specified!"
        - define tickutil_commands.ArgManager.panic true
        - goto fatal_error

    - foreach <[FlagArgs.linear_args].if_null[<list>]> as:value:
        - define tickutil_commands.ArgManager.linear <[tickutil_commands.ArgManager.linear_args].values.get[<[loop_index]>].if_null[null]>

        - if <[tickutil_commands.ArgManager.linear]> == null:
            - define "tickutil_commands.ArgManager.error_messages:|:'<[value].custom_color[emphasis]>' could not be parsed as a valid argument."
            - foreach next

        - if <[tickutil_commands.ArgManager.linear_args].values.get[<[loop_index]>].get[spread].if_null[false]>:
            - define value <[FlagArgs.linear_args].get[<[loop_index]>].to[last].separated_by[ ]>

        - if <[tickutil_commands.ArgManager.linear.accepted].parsed.if_null[true]>:
            - define arg <[arg].include[<[tickutil_commands.ArgManager.linear_args].keys.get[<[loop_index]>]>=<[value]>]>

        - else:
            # Generate a suggestion "(Did you mean '...'?)"
            - define tickutil_commands.ArgManager.suggestion <[tickutil_commands.ArgManager.linear.suggestions].if_null[<[tickutil_commands.ArgManager.linear.tab completes]>].parsed.as[list].closest_to[<[value]>]>

            - if <[tickutil_commands.ArgManager.suggestion].to_lowercase.difference[<[value].to_lowercase>]> <= 3 && <[tickutil_commands.ArgManager.suggestion]> != <empty>:
                - define tickutil_commands.ArgManager.suggestion_message " <&[base]>(Did you mean '<[tickutil_commands.ArgManager.suggestion].custom_color[emphasis]>'?)"

            - define "tickutil_commands.ArgManager.error_messages:|:'<[value].custom_color[emphasis]>' is not a valid value for the <[tickutil_commands.ArgManager.linear.required].parsed.if_null[false].if_true[required ].if_false[].custom_color[emphasis]>arg '<[tickutil_commands.ArgManager.linear_args].keys.get[<[loop_index]>].proc[GenerateSingularUsage_CM].context[<[tickutil_commands.ArgManager.linear_args].values.get[<[loop_index]>]>].custom_color[emphasis]>'!<[tickutil_commands.ArgManager.suggestion_message].if_null[]>"

            - if <[tickutil_commands.ArgManager.linear.required].parsed.if_null[false]>:
                - define tickutil_commands.ArgManager.panic true

                - if <script[CommandManager_Data].parsed_key[instantly terminate command execution on fatal error]>:
                    - goto fatal_error
    ...
    - define tickutil_commands.ArgManager.prefixed_args <[args].filter_tag[<[filter_value.type].equals[prefixed].if_null[false]>]>

    # Check if prefixed args are valid, and if so, add them to the arg map
    - foreach <[tickutil_commands.ArgManager.prefixed_args]> key:argname as:map:
        - foreach <[FlagArgs.prefixed_args].if_null[<map>]> key:flag as:value:
            - if <[flag]> == <[argname]>:
                - if <[map.accepted].parsed.if_null[true]>:
                    - define arg.<[argname]> <[value]>
                    - define FlagArgs.prefixed_args <[FlagArgs.prefixed_args].exclude[<[argname]>]>

                - else:
                    - if <[map].keys.contains_any[suggestions|tab completes]>:
                        - define tickutil_commands.ArgManager.suggestion <[map.suggestions].if_null[<[map.tab completes]>].parsed.as[list].closest_to[<[value]>]>

                        - if <[tickutil_commands.ArgManager.suggestion].difference[<[value]>]> <= 3 && <[tickutil_commands.ArgManager.suggestion]> != <empty>:
                            - define tickutil_commands.ArgManager.suggestion_message " <&[base]>(Did you mean '<[tickutil_commands.ArgManager.suggestion].custom_color[emphasis]>'?)"

                    - define "tickutil_commands.ArgManager.error_messages:->:Invalid prefixed argument '<element[-<[argname]><[sep]><[value]>].custom_color[emphasis]>' for arg '<[argname].proc[GenerateSingularUsage_CM].context[<[map]>].custom_color[emphasis]>'!<[map.explanation].exists.if_true[ Expected: '<[map.explanation].parsed.custom_color[emphasis]>'].if_false[]><[tickutil_commands.ArgManager.suggestion_message].if_null[]>"

    # Check for unknown prefixed args
    - foreach <[FlagArgs.prefixed_args].if_null[<map>]> key:argname as:val:
        - if <[tickutil_commands.ArgManager.prefixed_args].keys> contains <[argname]>:
            - foreach next

        - define tickutil_commands.ArgManager.suggestion <[tickutil_commands.ArgManager.prefixed_args].keys.closest_to[<[argname]>]>

        - if <[tickutil_commands.ArgManager.suggestion].difference[<[argname]>]> <= 3 && <[tickutil_commands.ArgManager.suggestion]> != <empty>:
            - define tickutil_commands.ArgManager.suggestion_message " <&[base]>(Did you mean '<[tickutil_commands.ArgManager.suggestion].proc[GenerateSingularUsage_CM].context[<[tickutil_commands.ArgManager.prefixed_args.<[tickutil_commands.ArgManager.suggestion]>]>].custom_color[emphasis]>'?)"

        - define "tickutil_commands.ArgManager.error_messages:->:Unknown prefixed argument '<element[-<[argname]>].custom_color[emphasis]>'!<[tickutil_commands.ArgManager.suggestion_message].if_null[]>"
    ...
    - if <player.exists>:
        - foreach <[args]> key:argname as:map:
            ...
            - if <[map.permissions].parsed.as[list].filter_tag[<player.has_permission[<[filter_value]>].not>].any>:
                - define "tickutil_commands.ArgManager.error_messages:->:You do not have permission to use the argument '<[argname].proc[GenerateSingularUsage_CM].context[<[map]>].custom_color[emphasis]>'!"
                - define arg.<[argname]>:!
    ...
    - if !<[tickutil_commands.ArgManager.error_messages].is_empty>:
        - narrate "<script[CommandManager_Data].parsed_key[formatting.lang.hline]><n><&[error]>There <[tickutil_commands.ArgManager.error_messages].size.is_more_than[1].if_true[were].if_false[was]> <[tickutil_commands.ArgManager.error_messages].size.color[white]> error message<[tickutil_commands.ArgManager.error_messages].size.is_more_than[1].if_true[s].if_false[]> when parsing the arguments for command <white>/<context.alias><&[error]>!<n.repeat[2]><[tickutil_commands.ArgManager.error_messages].parse_tag[<white><[parse_value]>].separated_by[<n.repeat[2]>].color[white]><n.repeat[2]><white>Usage: <&[base]><queue.script.proc[GenerateUsage_CM]><n><white>Attempted: <&[base]>/<context.alias> <context.raw_args><n.repeat[2]><[tickutil_commands.ArgManager.panic].if_null[false].if_true[<&[warning]>Was fatal! Aborting command execution!].if_false[<&[warning]>Was not fatal, continuing execution...]><n><script[CommandManager_Data].parsed_key[formatting.lang.hline]>"

        - if <[tickutil_commands.ArgManager.panic].if_null[false]>:
            - stop

    - define tickutil_commands.ArgManager:!
