...
DailyOrderRefresh:
    ...
    debug: false
    script:
    - foreach <script[ItemOrderWeightings].data_key[items]> key:item as:amount:

        # Low-weighting random variable
        - define randomChance <util.random.decimal[0].to[<util.random.decimal[0.5].to[1]>]>
        - define itemWeight <script[ItemOrderWeightings].data_key[items.<[item]>]>

        - if <[itemWeight]> > <[randomChance]>:
            - foreach next

        - define itemGradient <script[ItemGradientData].data_key[items.<[item]>]>
        - define itemAmountWeight <util.random.int[32].to[<[itemGradient].div[2]>].div[2].round_up.if_null[1]>
